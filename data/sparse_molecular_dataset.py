...
class SparseMolecularDataset():
    ...
    def next_train_batch(self, batch_size, z_dim):
        """
        desc:
        """
    ...
    def next_validation_batch(self, batch_size, z_dim):
        """
        desc:
        """
...
if __name__ == '__main__':
    ...
    """
    TODO set this shit up for reading in num_atoms from config file
    experiment_config = yaml.load(open(os.path.join(paths.EXPERIMENTS_DIR,
  	                                          cli_args.experiment_config),
                                      "rb"))

    """
    data.generate('gdb9.sdf', filters=lambda x: x.GetNumAtoms() <= 40)
